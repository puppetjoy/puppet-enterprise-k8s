...
def log(message):
    print(f"[console-selector] {message}", flush=True)
...
class ConsoleSelector:
    def __init__(self):
        self.pod_name = os.environ.get("CONSOLE_SELECTOR_POD_NAME", "").strip()
        self.namespace = os.environ.get("CONSOLE_SELECTOR_POD_NAMESPACE", "").strip()
        self.statefulset_name = os.environ.get("CONSOLE_SELECTOR_STATEFULSET_NAME", "").strip()
        self.label_selector = os.environ.get("CONSOLE_SELECTOR_LABEL_SELECTOR", "").strip()
        self.active_label_key = os.environ.get("CONSOLE_SELECTOR_ACTIVE_LABEL_KEY", "").strip()
        self.active_label_value = os.environ.get("CONSOLE_SELECTOR_ACTIVE_LABEL_VALUE", "").strip() or "true"
        self.poll_interval_seconds = env_int("CONSOLE_SELECTOR_POLL_INTERVAL_SECONDS", 5)

        for key, value in (
            ("CONSOLE_SELECTOR_POD_NAME", self.pod_name),
            ("CONSOLE_SELECTOR_POD_NAMESPACE", self.namespace),
            ("CONSOLE_SELECTOR_STATEFULSET_NAME", self.statefulset_name),
            ("CONSOLE_SELECTOR_ACTIVE_LABEL_KEY", self.active_label_key),
        ):
            if not value:
    ...
    def reconcile(self):
        ...
        if current_value != desired_value:
            ...
            log(f"{action} console label on {self.namespace}/{self.pod_name}")

        if selected_name != self.last_selected_name:
            if selected_name:
                log(f"Selected active console pod {self.namespace}/{selected_name}")
            else:
                log("No ready control-plane pod available for console service")
            self.last_selected_name = selected_name

        if should_be_active != self.last_active:
            state = "active" if should_be_active else "standby"
            log(f"{self.namespace}/{self.pod_name} is now {state} for console traffic")
